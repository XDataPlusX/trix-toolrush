...
| M4 | files-лейн | ✅ v0.3.0 — 12 патчей (rg-транспорт + strict-JSON `_hint` + стабильная пагинация + мемоизация гарда; ряды 4/6/7 Windows-гейтинга чтения пропущены осознанно — на Linux native read уже в апстриме), 30 тестов-дифференциалов. **Честный вердикт:** на тёплом Linux транспортный налог ≈ 0 (search 1.01x, no-match 1.00x, discovery 1.02x — paired, native подтверждён no-shell тестом); 5–7x оригинала были Windows-налогом. Лейн оставлен ради корректности и машин с дорогим spawn | тесты green; конверты идентичны |
| M5 | admission | ✅ ЗАКРЫТ как устаревший — субагент-исследование: апстрим удалил `_is_readonly_terminal_command`, terminal теперь name-gated sequential barrier (`_PARALLEL_SAFE_TOOLS`, tool_dispatch_helpers.py:48–61, fallthrough :268), контракт закреплён тестом test_tool_batch_segmentation.py:552. Классификатору нет места применения — патч не портится | вердикт в evidence/report.md |
| M6 | shell-лейн | ✅ v0.4.0 — Linux-брокер (bash -s, killpg, POSIX keep-list, os.replace коммит), 11 тестов. **Честный вердикт:** 1.0–1.17x на тёплом Linux (23.6x оригинала — Windows spawn-налог); лейн opt-in (`warm_shell: true` — для контейнеров/медленных spawn) | warm-сьют green |
| M7 | бенчмарки | ✅ evidence/report.md: paired search/terminal замеры, negative control, вердикты лейнов | отчёт в репо |
| M8 | Docker | ✅ payload-313.json собран (авто-выбор по рантайму); флот юзера: gateway локальный + py3.11-workhorse контейнеры как terminal backend → LocalEnvironment-лейны инертны там by design (fail-closed). Официальный образ: путь задокументирован в docs/DOCKER.md, in-image smoke при первом реальном деплое | payload-313 в репо |
| M9 | мультиверсии | ✅ решено механизмом: per-Python payload выбор в compat.py + rebuild_payload.sh против любого чекаута с выбором baseline-ref; неизвестный дрейф → громкая деградация лейна | механизм в проде |
