...
| M1 | Каркас | репо, plugin.yaml, `__init__.py`, портированный compat.py, `build_payload.py`, doctor.py с параметризованными путями; pytest в venv | `doctor.py` (без --smoke) на живом дереве: все лейны `compatible` или честно `degraded`; плагин бутится в свежем процессе через настоящий PluginManager |
| M2 | snapshot-лейн | применить 4 патча (уже PATCHABLE) + портировать `test_toolrush_snapshot_failclosed.py` (он уже dual-platform) | тесты green в свежем процессе; дисковые хэши дерева неизменны |
| M3 | rpc-лейн | 5 патчей + `test_toolrush_rpc.py` (portable) + parallel() в execute_code | `doctor --smoke`: реальный `execute_code` → 2 read_file через parallel(), exit 0 |
